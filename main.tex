...
Among these, two Javascript API were implemented successively in web \mbox{browsers :} XMLHttpRequest and \ws. % two APIs on met le pluriel aux abbrevs
...
\subsection{The periodic polling}
...
\subsection{Periodic vs long polling}
...
In Fig.~\ref{poll_vs_lpoll}, on the left, the client periodically polls the server to retrieve updates. This is shown on the figure as the ``polling interval''. When an update occurs on the server side, the client has to wait for the next polling to become aware of it. On the right, with long polling, the server keeps the connection open until the update occurs. The periodic polling introduces a delay that long polling can avoid. Also, in this figure, we can see that periodic polling creates superfluous network trafic as the first poll doesn't give new input. % pq polling au lieu de poll(s) parfois? the next poll, the first poll, ...
...
\ws{} has extended this header with custom \ws{} headers to perform the negotiation. For instance, the version of the \ws{} protocol to be used for the session will be negotiated during this process \cite{HighPerfBrowserNetworking:websocket}.
...
When a measurement is available, the server sends the message immediately from the sensor to the client, which closes the connection.% sends the message immediately inversion des 2 mots est mieux pour la lisibility

...
This expermient was conducted in \cite{roadblock}.
The setup is a simple web application that is a simple data echo service.
Clients send a fixed-size chunk of random data to the server who simply send it back to his respective client.
Figure~\ref{fig:road} shows the average delays in receiving consecutive data chunks as a function of data chunck size. 
This experiment is conducted on a 3G cellular data network with three differents protocols : XHR long polling, \ws{} and TCP.
...
Authors of this study conjecture that the reasons for this additional delay are due to additional buffering time at the client, more chatty protocols, and additional overhead.
The first point is mentioned in the IETF HTML5 websocket specification ``the implementation MAY delay the actual transmission arbitrarily, e.g., buffering data so as to send fewer IP packets.'' \cite{rfc6455}.
Also, we can notice that the standard deviation is larger for \ws{} compared to TCP.
This standard deviation is called the jitter mesurement in networking.
Authors of \cite{roadblock} make another experiment to mesure the jitter of each of these protocols.
\begin{figure}[!ht]
    \centering
...
The figure show that, with TCP, we can expect that more than 90\% of all data chunks will arrive at the client with an inter-chunk delay of 360ms or less. The corresponding numbers for \ws{} and XHR long polling are $760$ms and $1382$ms respectively.
We can see a large performance gap between TCP, \ws{} and XHR long polling.

\subsection{Comparing the throughput}
